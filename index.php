...
if ($databaseUrl) {
    ...
    if (!$parts || empty($parts['host'])) throw new RuntimeException('Invalid DATABASE_URL.');
    $dbName = isset($parts['path']) ? ltrim($parts['path'], '/') : 'railway';
    $dsn = 'pgsql:host=' . $parts['host'] . ';port=' . ($parts['port'] ?? 5432) . ';dbname=' . $dbName;
    $db = new PDO($dsn, $parts['user'] ?? null, $parts['pass'] ?? null);
} else {
    $host = getenv('PGHOST');
    if (!$host) throw new RuntimeException('PostgreSQL is not configured. DATABASE_URL/PGHOST is missing.');
    $dsn = 'pgsql:host=' . $host . ';port=' . (getenv('PGPORT') ?: 5432) . ';dbname=' . (getenv('PGDATABASE') ?: 'railway');
...
if($text==='/safonof' || $text==='/admin'){if(!admin($id)){send($chat,'⛔ Доступ запрещён.');exit;}send($chat,'👑 <b>АДМИН-ПАНЕЛЬ</b>',adminMenu($id));exit;}
