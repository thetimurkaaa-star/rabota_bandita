...
if ($databaseUrl) {
    ...
    $dbName = ltrim((string)($parts['path'] ?? '/railway'), '/');
    $dsn = 'pgsql:host=' . $parts['host'] . ';port=' . (int)($parts['port'] ?? 5432) . ';dbname=' . $dbName;
    $db = new PDO($dsn, isset($parts['user']) ? urldecode($parts['user']) : null, isset($parts['pass']) ? urldecode($parts['pass']) : null);
} else {
    $host = getenv('PGHOST');
    if (!$host) {
        throw new RuntimeException('PostgreSQL is not configured. Add Railway PostgreSQL and connect DATABASE_URL.');
    }
    $dsn = 'pgsql:host=' . $host . ';port=' . (int)(getenv('PGPORT') ?: 5432) . ';dbname=' . (getenv('PGDATABASE') ?: 'railway');
    $db = new PDO($dsn, getenv('PGUSER') ?: 'postgres', getenv('PGPASSWORD') ?: '');
}
$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
try {
    $schema = file_get_contents(__DIR__ . '/schema.sql');
    if ($schema === false) throw new RuntimeException('schema.sql is missing.');
    $db->exec($schema);
} catch (Throwable $e) {
    throw new RuntimeException('Database initialization failed: ' . $e->getMessage(), 0, $e);
}

$seed=$db->prepare('INSERT INTO vip_tasks(title,task_type,target_count,reward_money,reward_respect,min_vip,created_at) VALUES(?,?,?,?,?,?,?) ON CONFLICT DO NOTHING');
...
function ensureUser(array $from): void {
    ...
    if($id===MAIN_ADMIN_ID) $db->prepare("INSERT INTO admins(user_id,role,added_at) VALUES(?,?,?) ON CONFLICT(user_id) DO NOTHING")->execute([$id,'main',$now]);
    $db->prepare('INSERT INTO player_state(user_id) VALUES(?) ON CONFLICT(user_id) DO NOTHING')->execute([$id]);
    expireVip($id);
}
...
function addRespect(int $id,int $amount,string $reason,?int $actor=null): bool {
    global $db; if(!user($id)||$amount===0)return false; $db->prepare('UPDATE users SET respect=GREATEST(0,respect+?) WHERE id=?')->execute([$amount,$id]);
...
function grantVip(int $id,int $tier,int $days,?int $actor=null): bool {
    global $db; $u=user($id); if(!$u||$tier<1||$tier>5)return false;
    $now=time(); $currentTier=(int)$u['vip_tier']; $currentUntil=(int)$u['vip_until'];
    if($days===0) $until=0;
    elseif($currentTier===$tier && $currentUntil>$now) $until=$currentUntil+$days*86400;
    else $until=$now+$days*86400;
    $db->prepare('UPDATE users SET vip_tier=?,vip_until=? WHERE id=?')->execute([$tier,$until,$id]);
    if($actor!==null)logAdmin($actor,'Выдача VIP',"user={$id};tier={$tier};days={$days}"); return true;
}
...
function incrementTasks(int $userId,string $type,int $count=1): void {
    ...
    foreach($q as $t){
        ...
        $db->prepare('UPDATE vip_task_progress SET progress=LEAST(?,progress+?) WHERE task_id=? AND user_id=? AND period_key=? AND completed=0')->execute([(int)$t['target_count'],$count,(int)$t['id'],$userId,$period]);
...
function chatsText(): string { global $db;$rows=$db->query('SELECT * FROM chats WHERE is_active=1 ORDER BY LOWER(title)')->fetchAll(); foreach($rows as &$r){$cm=tg('getChatMemberCount',['chat_id'=>(int)$r['chat_id']]);if(!empty($cm['ok'])){$r['member_count']=(int)$cm['result'];$db->prepare('UPDATE chats SET member_count=?,updated_at=? WHERE chat_id=?')->execute([$r['member_count'],time(),(int)$r['chat_id']]);}} unset($r);$s="🏠 <b>ЧАТЫ БОТА</b>\n\n";if(!$rows)return$s.'Бот пока не видит подключённых чатов.';$i=1;foreach($rows as $r){$s.="{$i}. 🏠 <b>".esc($r['title']?:'Без названия')."</b> — <code>{$r['chat_id']}</code> ".((int)$r['can_send']?'✅':'❌')." — 👥 ".(int)$r['member_count']."\n";$i++;}return$s."\nВсего: <b>".count($rows)."</b>"; }
...
ensureUser($from);
if(in_array($chatData['type']??'', ['group','supergroup','channel'],true)){upsertChat($chatData,true);if(!($from['is_bot']??false) && (isset($m['text'])||isset($m['caption'])||isset($m['photo'])||isset($m['video'])||isset($m['video_note'])||isset($m['sticker'])||isset($m['document'])||isset($m['animation']))){recordActivity($chat,$id);incrementTasks($id,'chat');}}
...
if($text==='/balance'){$u=user($id);send($chat,"💰 <b>ТВОЙ БАЛАНС</b>\n\n💰 Бабки: <b>".money((int)$u['balance'])."</b>");exit;}
if($text==='/top'){$rows=$db->query('SELECT id,username,first_name,balance FROM users ORDER BY balance DESC LIMIT 10')->fetchAll();$s="🏆 <b>ТОП-10 ПО БАЛАНСУ</b>\n\n";$i=1;foreach($rows as $r){$name=$r['username']?'@'.$r['username']:($r['first_name']?:$r['id']);$s.="{$i}. ".esc($name)." — 💰 ".money((int)$r['balance'])."\n";$i++;}send($chat,$s);exit;}
if(preg_match('~^/ограбитьмагазин\s+(\d+)$~u',$text,$mm)){
    $owner=(int)$mm[1];
    if($owner===$id){send($chat,'❌ Нельзя ограбить свой магазин.');exit;}
    $shopOwner=user($owner);
    if(!$shopOwner){send($chat,'❌ Игрок не найден.');exit;}
    $q=$db->prepare('SELECT * FROM shops WHERE user_id=?');$q->execute([$owner]);$shop=$q->fetch();
    if(!$shop){send($chat,'❌ У этого игрока нет своего магазина.');exit;}
    $now=time();
    if($now-(int)$shop['last_robbed_at']<259200){$left=259200-($now-(int)$shop['last_robbed_at']);$days=max(1,(int)ceil($left/86400));send($chat,"⏳ Этот магазин уже грабили. Следующая попытка будет доступна примерно через <b>{$days} дн.</b>");exit;}
    $attacker=user($id);
    $weaponCount=(int)$db->query("SELECT COALESCE(SUM(quantity),0) FROM inventory WHERE user_id=".(int)$id." AND item_key IN ('knife','pistol','rifle')")->fetchColumn();
    $armor=(int)$db->query("SELECT COALESCE(quantity,0) FROM inventory WHERE user_id=".$owner." AND item_key='armor'")->fetchColumn()>0;
    $chance=max(5,min(90,45+(int)$attacker['strength']*0.30+$weaponCount*5-(int)$shopOwner['strength']*0.20-($armor?15:0)+vipPercent((int)$attacker['vip_tier'],'rob')));
    $success=random_int(1,100)<=round($chance);
    $available=(int)$shop['stored_income'];
    $amount=$available>0?min($available,max(1000,(int)floor($available*random_int(10,30)/100))):0;
    $db->prepare('UPDATE shops SET last_robbed_at=? WHERE user_id=?')->execute([$now,$owner]);
    if($success&&$amount>0){
        $db->prepare('UPDATE shops SET stored_income=stored_income-? WHERE user_id=? AND stored_income>=?')->execute([$amount,$owner,$amount]);
        changeBalance($id,$amount,'Ограбление магазина',$owner);
        $result='shop_success';
        send($chat,"🚨 <b>Магазин ограблен!</b>\n\n🏪 Владелец: <code>{$owner}</code>\n💰 Получено: <b>".money($amount)."</b>");
        send($owner,"🚨 <b>Твой магазин ограбили!</b>\n\n💸 Потеряно: <b>".money($amount)."</b>\n⏳ Следующее ограбление этого магазина возможно через 3 дня.");
    }else{
        $result=$success?'shop_empty':'shop_fail';
        send($chat,$success?'🏪 Магазин удалось взломать, но денег там не было.':'🚨 Ограбление магазина не удалось.');
    }
    $db->prepare('INSERT INTO robberies(attacker_id,target_id,type,result,amount,created_at) VALUES(?,?,?,?,?,?)')->execute([$id,$owner,'shop',$result,$amount,$now]);
    addXp($id,30);
    exit;
}
